    ...
    for trial in range(trials):
        ...
        while not valid:
            ...
            for step in range(timesteps +2):   # +2 bcs symos requires information at more time steps
                ...
                traj.append(obs)
            traj = np.stack(traj)
            if np.amax(traj[:, 2]) < env.MAX_VEL_1 - 0.001  and np.amin(traj[:, 2]) > -env.MAX_VEL_1 + 0.001:
                if np.amax(traj[:, 3]) < env.MAX_VEL_2 - 0.001  and np.amin(traj[:, 3]) > -env.MAX_VEL_2 + 0.001:
                    valid = True
                    x0.append(x)
        trajs.append(traj)
...
def arrange_DEL_dataset(data, u):
    ...
    x = np.concatenate((data[:,:-2, :d_f], data[:, 1:-1, :d_f], zero_controls[:, :-2], u[:, :-2], zero_controls[:, 1:-1], u[:, 1:-1], zero_controls[:, 2:],u[:, 2:]),2)
    y = data[:, 2:]
    return x, y, u
