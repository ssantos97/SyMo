...
def get_args():
    ...
    parser.add_argument('--num_angles', default=2, type=int, help='number of rotational coordinates')
...
def Train(args, x_train, y_train, u_train, x_test, y_test, u_test, data_int, controls_int):
    ...
    if (args.model == 'E2E-SyMo') and args.root_find == "Newton":
        ...
        x_train = torch.cat((x_train, zero_controls,  u_train), dim=1)
        train_data = TensorDataset(x_train, y_train)
        trainloader = DataLoader(train_data, batch_size=batch_size, shuffle=False)
        zero_controls = torch.zeros_like(u_test)
        x_test = torch.cat((x_test, zero_controls, u_test), dim=1)
    ...
    if args.model == 'SyMo' or args.model == 'E2E-SyMo':
        ...
        E_int_true = torch.tensor(acrobot().energy(data_int['x0']))
        ...
        for i in range(args.N_int):
            ...
            y_int_pred[i] = y_pred
            H_true = acrobot().Inertia_matrix(y_int_true[i].cpu().numpy())
            E = acrobot().energy(y_int_pred[i])
...
if __name__ == "__main__":
    ...
    methods = ['E2E-SyMo', 'L-NODE', 'NODE', 'SyMo']
