...
def get_args():
    ...
    parser.add_argument('--time_step', default=0.05, type=float, help='time_step')
    ...
    parser.add_argument('--backward_tol', default=1e-7, type=float, help="If Broyden specify a backward tolerance")
    ...
    parser.add_argument('--int_maxiter', default=10, type=int, help="Integration maxiter for Symos")
...
def Train(args, x_train, y_train, u_train, x_test, y_test, u_test, data_int, controls_int):
    ...
    if args.model == 'SyMo' or args.model == 'E2E-SyMo':
        ...
        E_int_true = cartpole().energy(data_int['x0'])
        ...
        for i in range(args.N_int):
            ...
            y_int_pred[i] = y_pred
            H_true = cartpole().Inertia_matrix(y_int_true[i].cpu().numpy())
    ...
    int_loss_per_traj_poses = torch.mean(losses, [1,2])
...
if __name__ == "__main__":
    ...
    dir = DATA_DIR + "/h=0.05/dataset.pkl"
    data = from_pickle(dir)
    n_trajs = 32
    noise_std = [0.0001, 0.0005, 0.001]
    ...
    dummy_controls_train = controls['train_u'][:n_trajs] + np.random.randn(*controls['train_u'][:n_trajs].shape)*noise_std[0]*0
    dummy_data_test = data['test_x'][:n_trajs] + np.random.randn(*data['test_x'][:n_trajs].shape)*noise_std[0]
    dummy_controls_test = controls['test_u'][:n_trajs] + np.random.randn(*controls['test_u'][:n_trajs].shape)*noise_std[0]*0
    ...
    dummy_controls_train = controls['train_u'][:n_trajs] + np.random.randn(*controls['train_u'][:n_trajs].shape)*noise_std[1]*0
    dummy_data_test = data['test_x'][:n_trajs] + np.random.randn(*data['test_x'][:n_trajs].shape)*noise_std[1]
    dummy_controls_test = controls['test_u'][:n_trajs] + np.random.randn(*controls['test_u'][:n_trajs].shape)*noise_std[1]*0
    ...
    dummy_controls_train = controls['train_u'][:n_trajs] + np.random.randn(*controls['train_u'][:n_trajs].shape)*noise_std[2]*0
    dummy_data_test = data['test_x'][:n_trajs] + np.random.randn(*data['test_x'][:n_trajs].shape)*noise_std[2]
    dummy_controls_test = controls['test_u'][:n_trajs] + np.random.randn(*controls['test_u'][:n_trajs].shape)*noise_std[2]*0
    ...
    for i in range(len(data_array)):
        ...
        methods = ["SyMo", "E2E-SyMo", 'L-NODE', 'NODE']
