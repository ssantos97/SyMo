...
def get_args():
    ...
    parser.add_argument('--int_maxiter', default=20, type=int, help="Integration maxiter")
...
def Train(args, x_train, y_train, u_train, x_test, y_test, u_test):
    ...
    if args.model == 'SyMo' or args.model == 'E2E-SyMo':
        ...
        for i in range(args.N_int):
            ...
            y_pred = implicit_integration_DEL(args, nn.eval(), x0_int[i], u_int[i], args.int_tol, args.int_maxiter)
            y_int_pred[i] = y_pred
        ...
        for i in range(args.N_int):
            y_pred = integrate_ODE(args, nn.eval(), x0_int[i], u_int[i])
            y_int_pred[i] = y_pred
            E = pendulum().energy(y_int_pred[i])
            E_loss_per_traj.append(torch.mean((E_int_true[i] - E)**2))
            Energies[i] = E.view(len(E), 1)
    ...
    int_loss_per_traj = torch.mean(losses, [1])[:, 1]
    ...
    stats['E_int_true'] = E_int_true.numpy()
...
if __name__ == "__main__":
    ...
    x0_min = [-np.pi, -2*np.pi, -10] #q, q_dot, u_magnitude
    x0_max = [np.pi, 2*np.pi, 10]
    ...
    methods = ['E2E-SyMo', 'SyMo','LODE', 'NODE']
